...
class RoundUpQuantizer(nn.Module):
    def __init__(
        self,
        n_bits: int = 8,
        group_size=None,
...
class RoundDownQuantizer(nn.Module):
    def __init__(
        self,
        n_bits: int = 8,
        group_size=None,
    ...
    def fake_quant(self, x):
        scale = clamp_ste(self.scale,1e-4, 1e4)
        round_zero_point = clamp_ste(round_down_ste(self.zero_point), self.qmin, self.qmax)
        
        dim1, dim2 = x.shape
        x = x.reshape(-1, self.group_size)
        x_int = round_down_ste(x / scale)
        if round_zero_point is not None:
            x_int = x_int.add(round_zero_point)
        x_int = x_int.clamp(self.qmin, self.qmax)
        x_dequant = x_int
